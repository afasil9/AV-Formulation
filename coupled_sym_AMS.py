...
from ufl import SpatialCoordinate, as_vector, sin, pi, curl, diff, div,ds, dx, grad, inner, curl
...
w1 = ufl.TrialFunction(V1)
v1 = ufl.TestFunction(V1)

a00 = dt*nu*ufl.inner(curl(u), curl(v)) * dx + sigma*ufl.inner(u, v) * dx
L0 = dt* ufl.inner(f0, v) * dx + sigma*ufl.inner(u_n, v) * dx + sigma*ufl.inner(v, grad(w_n)) * dx

a01 = sigma*ufl.inner(grad(w1), v) * dx
a10 = sigma*ufl.inner(grad(v1), u) *dx

a11 = dt * ufl.inner(sigma*ufl.grad(w1), ufl.grad(v1)) * ufl.dx
L1 = dt * f1 * v1 * ufl.dx + sigma*ufl.inner(grad(v1),u_n) *dx + sigma*ufl.inner(grad(v1), grad(w_n)) * dx
