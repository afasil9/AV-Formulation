...
# f1 = Constant(domain, default_scalar_type(-9))

f0 = curl(curl(uex)) + sigma * du_ex_dt + sigma * grad(uex1)
f1 = -div(f0)
...
a00 = dt * nu * inner(curl(u), curl(v)) * dx + inner((u * sigma), v) * dx
L0 = (
    dt * inner(f0, v) * dx
    + sigma * inner(u_n, v) * dx
    - dt * inner(sigma * grad(uex1), v) * dx
)

a11 = inner(grad(u1), grad(v1)) * dx
L1 = f1 * v1 * dx + inner(div(du_ex_dt), v1) * dx

a = form([[a00, None], [None, a11]])
...
par_print(comm, f"L2 norm of u: {L2_norm(u_n - uex)}")
par_print(comm, f"L2 norm of u1: {L2_norm(u_n1 - uex1)}")

par_print(comm, f"L2 norm of E: {L2_norm(E - E_exact)}")
par_print(comm, f"L2 norm of B: {L2_norm(B - curl(uex))}")

# print(L2_norm(E - E_exact))
# print(L2_norm(B - curl(uex)))
