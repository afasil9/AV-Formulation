...
from generate_team30_meshes_3D import mesh_parameters, model_parameters, surface_map

__all__ = ["update_current_density"]
...
class DerivedQuantities3D:
    ...
    def __init__(
        ...
        # Derived quantities
        self.B = ufl.curl(Az)  # Electromagnetic field
        self.Bphi = ufl.inner(self.B, ufl.as_vector((-x[1], x[0]))) / r
        self.Br = ufl.inner(self.B, x) / r
        self.E = -(Az - Azn) / self.dt  # NOTE: as grad(V)=dV/dz=0 in 2D (-ufl.grad(V)) is excluded
        self.Ep = self.E + _cross_2D(u, self.B)
...
def compute_loss(comm, A_out, A_prev, dt, sigma,dx ,domains):
    E = -(A_out - A_prev) / dt
